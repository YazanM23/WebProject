<?php
    session_start();
    if (isset($_SESSION['isRecipient'])){
        if ($_SESSION['isRecipient'] == 1){
            $id = $_SESSION['id'];
...
//recipient
            if (isset($_POST['SaveR'])){
                $firstName = $_POST['FNR'];
                $secondName = $_POST['SNR'];
                $thirdName = $_POST['TNR'];
                $lastName = $_POST['LNR'];

                $username = $_POST['usernameDR'];
                $password = $_POST['passwordDR'];
                ...
                $bloodType = $_POST['BtypeR'];
                $noMentall = isset($_POST['NoMentalR']) ? "Yes" : "No";
                $isClearr = isset($_POST['isClearR']) ? "Yes" : "No";

                $diseases2 = array("NoMental"=>"$noMentall", "isClear"=>"$isClearr");
                ...
                street = '$street', username = '$username', password = '$password' WHERE id = '$id';";
                    ...
                    foreach ($diseases2 as $x => $x_value){
                        $queryStr3 = "UPDATE recipient_diseases SET infection = '$x_value' WHERE disease_name = '$x' AND recipient_id = '$recipient_id';";
                        $res3 = $db->query($queryStr3);
                        $db->commit();
                    }
                    //end of insertion
    ...
    <head>
        <script>
            function flipMessage(t){
                if(t == 0)
                {
                    document.getElementById('eba1').style.display = 'none';
                    document.getElementById('eba2').style.display = 'block';
                    document.getElementById('outerDiv2').style="animation: taa 0.5s;background: linear-gradient(160deg,white 20%,#59caa8);";
                }
                else{
                    document.getElementById('eba1').style.display = 'block';
                    document.getElementById('eba2').style.display = 'none';
        ...
        <style>
            #outerDiv2:hover{
                background: linear-gradient(160deg,white 20%,#59caa8);
            ...
            #outerDiv1:hover{
                background: linear-gradient(160deg,white 20%,#4895df);
    ...
    ";
        ...
        <link rel="stylesheet" href="styles/recipientStyling.css">
...
";
    ...
    <div class="navBar">
        ...
        <div id="title" style="display: inline-block" data-aos="fade-out"  data-aos-duration="1000"><a  href="recipient.php" style="text-decoration: none; color: white; font-style: normal;">PAODT</a></div>
        ...
        <ul class="navList" id="navList1">
            ...
            <li class="navItem"><a class = "navLink" href="ProfileR.php">Profile</a></li>
            <li class="navItem"><a class = "navLink" style="margin-left: 20px;" href="login.php">Log out</a></li>
    ...
    <h1 class="o1Text2" style="margin-left: 15px;" data-aos="fade-up" data-aos-duration="1500"><a href="#SignBox" class="signUp" >Messages</a></h1>
    ...
    <div class="box" id="SignBox" data-aos="fade-in">
        <div class="OuterDiv" id="outerDiv1" data-aos="fade-up" data-aos-duration="2900" style="" onclick="document.getElementById('overallid').style.display = 'block';">
            <div>
                <div style="margin-top: 80px;margin-bottom: 20px;color: #4895df"> <i class="fa-solid fa-envelope-open fa-5x"></i></div>
                <h1 style="font-size: 35px">Inbox</h1>
        ...
        <div class="OuterDiv" data-aos="fade-up" data-aos-duration="2900" data-aos-delay="200" id="outerDiv2" onclick="flipMessage(0)"  >
            <div id="eba1">
                <div style="margin-top:80px;margin-bottom: 20px;color: #59caa8"> <i class="fa-solid fa-envelope fa-5x"></i></div>
                <h1 style="font-size: 35px; clear: both">New message</h1>

            </div>
            <div id="eba2">
                <form action="recipient.php" method="post" autocomplete="off">
                    <div style="width: 90% ; margin: 20px auto;"  >
                        <input required type="text" id="titleOfMessage" placeholder="The title" name="titleOfMessage" style="width: 80% ; height: 30px ; box-sizing: border-box; border-radius: 10px"><br><br>
                        <textarea required name="bodyofMessage" id="bodyofmessage" placeholder="Your message here" style="padding: 10px; box-sizing: border-box; width: 80%;max-width: 80%; border-radius: 10px; min-height: 200px   ; max-height: 200px; height: 200px; min-width: 80%"></textarea>
                    </div>
                    <br>
            ...
            if(isset($_POST['sendbtn'])) {
                $a1 = $_POST['bodyofMessage'];
                $a2 = $_POST['titleOfMessage'];
                $aa = $_SESSION['recipient_id'];

                try {
                    $db = new mysqli('localhost','root','123456','project');

                    $gg = "insert into message (content,recipient_id,title,answered) values('$a1',$aa,'$a2','no')";
                    $ag = $db->query($gg);
                    $db->commit();
                    $db->close();
                } catch (Exception $e){
    ...
    <div class="overall" id="overallid">
        <button style="color: white; background: transparent;font-size: 20px;margin-left: 20px; border:none" onclick="document.getElementById('overallid').style.display = 'none';">back</button>
        <div class="inbox-container">
            <h1 style="font-size: 50px; color: white">Inbox</h1>
            <div class="in-inboxcontainer">
                ...
                <?php
                    ...
                    try{
                        ...
                        $id =  $_SESSION['recipient_id'];


                        $db = new mysqli('localhost','root','123456','project');
                        $bdquery = "SELECT * FROM message where recipient_id= $id order by message_number desc;";
                        $result = $db->query($bdquery);
                        ...
                        for($i = 0;$i<$size;$i++) {
                            $myMessages = $result->fetch_object();
                            $yazan = $myMessages->content;
                            $bara = $myMessages->reply;

                            echo "

    
                    <div style= 'margin-right: 10px;display:inline-block'><i class='fa-solid fa-envelope fa-3x' style='color:#4895df'></i></div>
                    <p class='message-bdair'>$yazan</p>

                    <br>

                    <div style='clear: both;margin-right: 10px;display: inline-block;padding-left: 40px'><i class ='fa-solid fa-reply fa-3x' style='color:#59caa8'></i></div>
                     <p class='answer-bdair'>$bara</p>
                    <br>
                ";
                        }
